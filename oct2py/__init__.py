...
round-trip to Octave (e.g. writing passing an int8 ndarray to Octave and
reading back the result gives an int8 ndarray with the same values).
Almost all Python types can be sent to Octave (including ndarrays of arbitrary
rank) and read back in the same form.  See tests/test_oct2py.py for the list
of TYPE_CONVERSIONS.  Currently the library does not support nested lists with
strings in them, or an ndarray with 'S' type fields of rank > 1.  Ndarrays of
dtype "object" are not implemented either.  Corner cases like sparse or empty
matrices have not been tested.  Note that dictionaries are mapped to
structures, which are returned as Struct objects::
...
shows the effect.  After a start-up time for the Octave engine (<1s),
...
See thread_test.py for an example of several objects writing a different value
for the same variable name simultaneously and successfully retrieving their
own result::
...
noticeable differences are nested functions are not allowed, and GUIs
(including uigetfile, etc.) are not supported. There are several Octave
packages (think toolboxes), including image and statistics
